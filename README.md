...
The way spread-cron detects changes on a resource and stores them is defined in the branch-specific `options` file. It is a plain shell file with two variables, `pattern_extractor` and `message`. `pattern_extractor` contains a shell command that scrapes an external resource and returns a meaningful value about the resource's status. For instance, in the core snap case this variable could contain a curl command that asks the search endpoint of the store for the snap info for a channel and architecture and extracts from the returned JSON the `revision` field.

After retrieving the state value from the external resource spread-cron compares it with the previous stored state. For storing resource state spread-cron also uses git facilities, each resource state is stored as a commit in the resource branch history. The commit text uses the `message` entry in the `options` file, plus the current state scrapped from the external resource. So, for knowing if there have been actual changes in the resource, spread-cron iterates through the commit history in the branch until it finds a commit message that matches "`message` (old_value)" and then compares the new value with old_value, if they are different then a new commit is issued with a message like "`message` (new_value)".
...
then spread-cron will iterate through the commit list until it finds the first entry which matches `$message` from the options file and extract the old version from there, in this case 549. Since 550 is different from 549, it will add a new empty commit in the current spread-cron branch with the message `New OS snap in beta channel (550)`.
...
After a new change is detected the full snapd repo is cloned from master into the target branch. spread will be executed from that directory after the suite customization.
