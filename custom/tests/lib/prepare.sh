...
prepare_classic() {
    ...
    #enable proposed
    echo "deb http://archive.ubuntu.com/ubuntu/ xenial-proposed restricted main multiverse universe" | tee -a /etc/apt/sources.list
    apt update
    # install snap-confine from proposed
    apt install snap-confine -y
